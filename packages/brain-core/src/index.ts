...
const routes: Route[] = [
  {
    intent: "message",
    pattern: /telegram|xabar yubor|jo['‘’]?nat/,
    confidence: 0.96,
    capabilities: ["messaging.telegram.send"],
  },
  {
    intent: "media",
    pattern: /instagram.*(rasm|video)|(?:rasm|video).*instagram/,
    confidence: 0.96,
    capabilities: ["media.video.generate", "social.instagram.publish"],
  },
  {
    intent: "media",
    pattern: /rasm|video|galereya/,
    confidence: 0.9,
    capabilities: ["media.video.generate"],
  },
  {
    intent: "reminder",
    pattern: /eslat|budilnik|signal|soat/,
    confidence: 0.94,
    capabilities: ["clock.alarm.create"],
  },
  {
    intent: "automation",
    pattern: /avtomat|har safar|agar.+bo['‘’]?lsa/,
    confidence: 0.9,
    capabilities: ["automation.create"],
  },
  {
    intent: "note",
    pattern: /qayd|yozib qo['‘’]?y|saqla/,
    confidence: 0.9,
    capabilities: ["notes.create"],
  },
  {
    intent: "task",
    pattern: /vazifa|topshiriq|bajar/,
    confidence: 0.82,
...
export function routeCommand(input: CommandInput): BrainDecision {
  const text = input.text.toLocaleLowerCase(
    input.locale.startsWith("uz") ? "uz" : undefined,
  );
  ...
  const clarificationQuestion = missingFields.includes("time")
    ? "Qaysi vaqtga o‘rnatay? Masalan, 17:00."
    : missingFields.includes("recipient")
      ? "Telegram recipientini @username ko‘rinishida ayting."
      : missingFields.includes("intent")
        ? "Bu buyruqdan qanday natija kutyapsiz?"
        : undefined;
  return {
    intent: route.intent,
    confidence: route.confidence,
    rationale: `Matched ${route.intent} route`,
    capabilityNames: route.capabilities,
    entities,
    missingFields,
